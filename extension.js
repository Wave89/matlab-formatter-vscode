...
class MatlabFormatter {
    ...
    format(document, range) {
        return new Promise((resolve, reject) => {
            ...
            let deleteEmptyLine = " --deleteEmptyLine=" + vscode.workspace.getConfiguration('matlab-formatter')['deleteEmptyLine'];
            ...
            var p = cp.exec(this.py + " " + this.formatter + " " + filename + indentwidth + separateBlocks + deleteEmptyLine + indentMode + addSpaces + matrixIndent + start + end, (err, stdout, stderr) => {
                if (stdout != '') {
